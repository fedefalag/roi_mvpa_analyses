...
opt = designtwo_getOption_individualAuditRoi;

% Loop through subjects to create individual ROIs
for sub = 1:length(opt.subjects)
    
    % where do you want to save the new mask
    outputDir = strcat(opt.maskPath,'sub-',opt.subjects{sub},'/task-voicelocalizerCombiemo/expand/');
    ...
    for roi = 1:length(opt.mask{sub}.coordinates)    
        ...
        if opt.mask{sub}.action{roi} == 1
            ...
            clusterMask = {strcat(opt.maskPath,'sub-',opt.subjects{sub},'/task-voicelocalizerCombiemo/sub',opt.subjects{sub},opt.mask{sub}.roiname{roi},'mask.nii')};
